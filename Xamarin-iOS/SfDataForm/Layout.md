---
layout: post
title: Layout | SfDataForm | Xamarin.iOS | Syncfusion<sup>&reg;</sup>
description: This section explains layouts and its customization functionalities in SfDataForm for Xamarin.iOS platform.
...
DataForm provides support for linear and grid layout. [DataFormLayoutManager](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormLayoutManager.html) creates [DataFormItemView](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormItemView.html), [DataFormGroupItemView](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormGroupItemView.html) and manages layout of label, editor and validation label.
...
By default, DataForm arranges one data field per row. It is possible to have more than one date field per row by setting [ColumnCount](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.SfDataForm.html#Syncfusion_iOS_DataForm_SfDataForm_ColumnCount) property which provide Grid like layout for DataForm.  
...
You can hide the label by defining [DisplayOptions](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DisplayOptionsAttribute.html) attribute or by handling `AutoGeneratingDataFormItem` event. In this case, editor only will be loaded.
...
Labels can be positioned either at the top or left side of editor. By using the [LabelPosition](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.SfDataForm.html#Syncfusion_iOS_DataForm_SfDataForm_LabelPosition) property, you can layout the label associated with editor.
...
You can change the label position using the [LabelPosition](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormItem.html#Syncfusion_iOS_DataForm_DataFormItem_LabelPosition) property in `DataFormItem`, and it will be handled in the `AutoGeneratingDataFormItem` event.
...
To show the image as label, you need to use [ImageSource](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DisplayOptionsAttribute.html#Syncfusion_iOS_DataForm_DisplayOptionsAttribute_ImageSource) property in [DisplayOptions](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DisplayOptionsAttribute.html) attribute. Images will be taken from …\Resources folder.
...
By using [ImageSource](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormItem.html#Syncfusion_iOS_DataForm_DataFormItem_ImageSource) property in `DataFormItem`, you can load image as label.
...
You can change the fields order by using [Order](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormItem.html#Syncfusion_iOS_DataForm_DataFormItem_Order) property in `DataFormItem`.
...
By setting `ColumnCount` property in DataForm, not grouped items only will be arranged in grid layout. If you want to load grid layout, you need to set [ColumnCount](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormGroupItem.html#Syncfusion_iOS_DataForm_DataFormGroupItem_ColumnCount) for [DataFormGroupItem](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormGroupItem.html).
...
By default, group will be loaded in expanded state. You can collapse the group by setting [IsExpanded](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormGroupItem.html#Syncfusion_iOS_DataForm_DataFormGroupItem_IsExpanded) property as false in [DataFormGroupItem](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormGroupItem.html).
...
You can set restrict the group being expanded or collapsed by setting [AllowExpandCollapse](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormGroupItem.html#Syncfusion_iOS_DataForm_DataFormGroupItem_AllowExpandCollapse) as `false` in [DataFormGroupItem](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormGroupItem.html).
...
You can expand or collapse the group programmatically by using [ExpandGroup](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.SfDataForm.html#Syncfusion_iOS_DataForm_SfDataForm_ExpandGroup_System_String_) and [CollapseGroup](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.SfDataForm.html#Syncfusion_iOS_DataForm_SfDataForm_CollapseGroup_System_String_) method respectively.
...
If you want to customize the layout, you need to override [DataFormLayoutManager](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormLayoutManager.html) and assign to [SfDataForm.LayoutManager](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.SfDataForm.html#Syncfusion_iOS_DataForm_SfDataForm_LayoutManager) property.
...
By using DataFormLayoutManager class , you can customize the generated label by overriding the [GenerateViewForLabel](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormLayoutManager.html#Syncfusion_iOS_DataForm_DataFormLayoutManager_GenerateViewForLabel_Syncfusion_iOS_DataForm_DataFormItem_) method and also you can customize the editor by overriding the `OnEditorCreated` method.Here, BackgroundColor and TextColor of label and editor is customized.
...
You can set label and editor width proportionally by using [LabelWidth](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.SfDataForm.html#Syncfusion_iOS_DataForm_SfDataForm_LabelWidth) and [EditorWidth](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.SfDataForm.html#Syncfusion_iOS_DataForm_SfDataForm_EditorWidth) properties.
...
You can set increase the row height by using [RowSpan](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DisplayOptionsAttribute.html#Syncfusion_iOS_DataForm_DisplayOptionsAttribute_RowSpan) property in `DisplayOptions` attribute.
...
When grid layout is used, you can increase the column width by using [ColumnSpan](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DisplayOptionsAttribute.html#Syncfusion_iOS_DataForm_DisplayOptionsAttribute_ColumnSpan) property in `DisplayOptions `attribute.
...
You can change the field visibility by using [IsVisible](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormItemBase.html#Syncfusion_iOS_DataForm_DataFormItemBase_IsVisible) property in `DataFormItem`.
...
You can define the height of each `DataFormItem` using the [Height](https://help.syncfusion.com/cr/xamarin-ios/Syncfusion.iOS.DataForm.DataFormItemBase.html#Syncfusion_iOS_DataForm_DataFormItemBase_Height) property, and it will be handled in the `AutoGeneratingDataFormItem` event.
